...
func getSliceEncoder() *sliceArrayEncoder {
	if ret, ok := _sliceEncoderPool.Get().(*sliceArrayEncoder); ok {
		return ret
	}
...
func getConsoleEncoder() *consoleEncoder {
	if ret, ok := _consolePool.Get().(*consoleEncoder); ok {
		return ret
	}
...
func (enc *consoleEncoder) AddReflected(key string, obj interface{}) error {
	enc.addKey(key)
	_, err := objPrinter.Fprint(enc.buf, obj)
	return err
}
...
func (enc *consoleEncoder) AppendReflected(val interface{}) error {
	_, err := objPrinter.Fprint(enc.buf, val)
	return err
}
...
func (enc *consoleEncoder) EncodeEntry(ent zapcore.Entry, fields []zapcore.Field) (*buffer.Buffer, error) {
	...
	for i := range arr.elems {
		if i > 0 {
			line.AppendString(tokenTab)
		}
		_, _ = objPrinter.Fprint(line, arr.elems[i])
	}
	putSliceEncoder(arr)
...
func (enc *consoleEncoder) writeContext(line *buffer.Buffer, extra []zapcore.Field) {
	context, ok := enc.Clone().(*consoleEncoder)
	if !ok {
		return
	}
